...
#NoTrayIcon
;@Ahk2Exe-ExeName VIREMA_0.4.1_installer.exe
;@Ahk2Exe-SetName VIREMA_0.4.1_installer
...
if not (A_IsAdmin) {
	...
	MsgBox "A_IsAdmin: " A_IsAdmin "`nCommand line: " DllCall("GetCommandLine", "str"), "VIREMA 0.4.1 installer"
	ExitApp
}

; INSTALLATION
if (MsgBox("Do you want to intall (reinstall) VIREMA 0.4.1 to this computer?", "VIREMA 0.4.1 installer", 4) = "Yes") {
	...
	Sleep(installer_sleep_ms)
	installer_progress_window_text.Text .= "Done`n"
	installer_progress_window_text.Text .= "`nSuccessfully installed VIREMA 0.4.1.`nA startup link was placed at the desktop."
	Sleep(installer_sleep_ms)
	installation_done_or_failed()
} else {
	MsgBox("Aborting installation...", "VIREMA 0.4.1 installer error", "T1.56")
	ExitApp
}

installation_done_or_failed(*) {
		Loop {
		if not WinExist("VIREMA 0.4.1 installer") {
			ExitApp
		}
		if WinExist("VIREMA 0.4.1 installer") and not WinActive("VIREMA 0.4.1 installer") {
			ExitApp
		}
		Sleep(156)
	}
